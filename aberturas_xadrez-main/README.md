...
Ao tocar em **“Ativar Stockfish profundo”**, o site baixa o Stockfish 18 single-threaded/light e pede uma busca UCI de profundidade 13 para a posição atual. A busca roda em um Web Worker para não congelar o tabuleiro. Se a conexão bloquear esse download, a interface informa o ocorrido e continua funcionando com a Maré Neural.
...
- **Motor local/produção**: hospede os arquivos `.js` e `.wasm` do Stockfish no seu próprio domínio e troque as duas URLs da ponte. Isso dá previsibilidade de cache, elimina a dependência de rede externa e facilita cumprir a licença.

## Notas de licença

O projeto não embute o binário do Stockfish. Quando ativado, ele solicita a versão leve, single-threaded, do projeto [nmrugg/stockfish.js](https://github.com/nmrugg/stockfish.js), distribuída sob GPL-3.0. Caso você hospede ou distribua esses arquivos junto do seu site, siga as obrigações da licença. Veja também [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
