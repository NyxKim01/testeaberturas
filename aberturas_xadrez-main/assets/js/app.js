...
import { PositionEvaluator, StockfishBridge, probabilitiesFromCentipawns } from "./core/position-evaluator.js";
...
function renderProbabilities(analysis) {
  ...
  ui.whiteProbability.textContent = pct(white);
  ui.drawProbability.textContent = pct(draw);
  ui.blackProbability.textContent = pct(black);
...
function refresh({ requestEngine = true } = {}) {
  ...
  if (requestEngine && engine.ready) {
    ...
    engine.evaluate(fen, currentTurn, (centipawns) => {
      if (fen !== game.fen()) return;
      renderAnalysis({ ...evaluator.evaluate(game), centipawns, probabilities: probabilitiesFromCentipawns(centipawns), source: "Stockfish 18" });
    });
  }
}
